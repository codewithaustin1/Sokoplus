  ...
  try {
    ...
    const response = await axios.post(
      "https://api.paystack.co/transaction/initialize",
      {
        ...
        metadata,
        currency: "KES",
        callback_url: callback_url || `${req.headers.origin || "https://sokoplus.com"}/payment-success`
      },
      {
        headers: {
...
`;
    ...
    if (isQuotaError) {
      quotaCooldownUntil = Date.now() + (10 * 60 * 1000); // 10 minutes cooldown
      console.warn("[SokoSmart] Gemini quota rate-limited (429) in index.ts. Operating in High-Performance Local Search mode.");
    } else {
