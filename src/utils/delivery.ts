...
export function calculateDelivery(
  ...
  if (country !== "Kenya") {
    tier = "EAC Cross-Border Priority";
    desc = "Dispatched via fast international land & air courier with custom clearance and tracking.";
    cutoffHour = 14; // 2:00 PM
    if (country === "Rwanda") {
      daysToAdd = 4;
    } else {
      daysToAdd = 3; // Uganda and Tanzania standard 3 days
    }
  } else {
...
export function calculateShippingFee(
  ...
  if (country !== "Kenya") {
    ...
    if (country === "Tanzania") return 1100;
    if (country === "Rwanda") return 1200;
    return 1500; // general fallback
  }
