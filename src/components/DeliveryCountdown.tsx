...
export const DeliveryCountdown: React.FC<DeliveryCountdownProps> = ({
  ...
  return (
    <div className={`p-5 rounded-2xl bg-gradient-to-br from-orange-50/10 via-orange-50/5 to-transparent border border-gray-100 shadow-sm ${className}`}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        {/* Delivery estimation info */}
        <div className="flex items-start gap-3.5">
          <div className="p-3 bg-orange-100/60 text-orange-600 rounded-full border border-orange-200/40 shadow-sm shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full select-none ${
                isNairobiCentral 
                  ? "bg-green-100 text-green-700 font-extrabold border border-green-200/20" 
                  : "bg-orange-100 text-orange-750 font-extrabold border border-orange-200/20"
              }`}>
                {prediction.tier}
              </span>
            ...
            <h4 className="text-lg font-black text-gray-900 mt-1.5 leading-snug">
              Delivered by <span className="text-orange-600">{prediction.time}</span>
            </h4>
        ...
        <div className="flex flex-col items-start sm:items-end justify-center py-2 px-4 rounded-xl bg-gray-50 border border-gray-100/60 min-w-[200px]">
          <span className="text-[10px] font-black uppercase text-gray-450 tracking-wider flex items-center gap-1">
            <Clock size={11} className={isUrgent ? "text-red-500 animate-spin" : ""} /> Dispatch Cutoff Timer
          </span>
          <div className="flex items-baseline gap-1 mt-1 font-mono">
            <span className={`text-2xl font-black tracking-tight ${isUrgent ? "text-red-600 animate-pulse" : "text-gray-800"}`}>
            ...
            <span className={`text-sm font-black tracking-tight text-gray-500`}>
      ...
      {!hideSelector && (
        <div className="mt-4 pt-3.5 border-t border-gray-100/80 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <button
              onClick={() => setIsChangingLocation(!isChangingLocation)}
              className="text-xs font-extrabold text-orange-650 hover:text-orange-700 transition-colors flex items-center gap-1 bg-transparent border-none cursor-pointer focus:outline-none"
            >
              <MapPin size={12} />
              <span>Shipping to: </span>
              <span className="text-gray-700 font-black border-b border-dashed border-gray-400 hover:border-orange-600 pb-0.5">
          ...
          <AnimatePresence>
            {isChangingLocation && (
              ...
              >
                <div className="bg-gray-50/50 p-4 rounded-xl border border-gray-100/60 grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1.5">
                  <div>
                    <label className="block text-[10px] font-black uppercase text-gray-400 mb-1 tracking-wider">County</label>
                    <select
                      value={selectedCounty}
                      onChange={handleCountyChange}
                      className="w-full text-xs font-bold p-2.5 bg-white border border-gray-200 rounded-lg outline-none focus:ring-1.5 focus:ring-orange-500/30 focus:border-orange-500 transition-all cursor-pointer"
                    >
                      {counties.map((c) => (
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase text-gray-400 mb-1 tracking-wider">City / Neighborhood</label>
                    <select
                      value={selectedCity}
                  ...
                  <div className="sm:col-span-2 flex justify-end">
                    <button
                      ...
                      className="px-3.5 py-1.5 text-xs font-bold text-white bg-gray-900 hover:bg-orange-600 transition-all rounded-lg cursor-pointer flex items-center gap-1"
                    >
